...
export interface ApiKeyRepository {
  ...
  // Scoped to merchantId too — a merchant can only ever revoke their own key, never guess at
  // someone else's public id. Absent for the wrong merchant comes back exactly like absent
  // entirely: see errorHandler's ResourceNotFoundError -> 404.
...
export function createApiKeyRepository(pool: Queryable): ApiKeyRepository {
  return {
    ...
    // Idempotent, and doesn't overwrite the original timestamp on a repeat call — same
    // don't-mutate-history discipline as the ledger, applied to a much smaller fact.
    async revoke(id) {
      const result = await pool.query<ApiKeyRow>(
